  ...
  document.getElementById('audit-heading').innerHTML = `
    <div class="audit-heading-top">
      <div>
        ...
        <h1 class="audit-heading-title">${escapeHtml(PROPOSAL_AGENT.name)}</h1>
        <p class="audit-heading-role">${escapeHtml(PROPOSAL_AGENT.role)}</p>
      </div>
...
function exportPdf(trail) {
  ...
  doc.innerHTML = `
    ...
    <div class="p-summary">
      <div><strong>Agent:</strong> ${escapeHtml(PROPOSAL_AGENT.name)}</div>
