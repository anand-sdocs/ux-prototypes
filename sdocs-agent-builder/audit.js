...
function getParams() {
  ...
  return { agentId: p.get('agent'), stepType: p.get('step') };
}
...
function init() {
  const { agentId, stepType } = getParams();
  const trail = AUDIT_TRAILS[agentId] || AUDIT_TRAILS[Object.keys(AUDIT_TRAILS)[0]];
  const agent = AGENTS.find(a => a.id === trail.agentId);
  ...
  renderHeading(trail, agent);
  renderFlowCanvas(trail, agent);
...
function renderHeading(trail, agent) {
  document.getElementById('audit-heading').innerHTML = `
    <div class="audit-heading-top">
      <div>
        ...
        <h1 class="audit-heading-title">${escapeHtml(agent.name)}</h1>
        <p class="audit-heading-role">${escapeHtml(agent.role)}</p>
      </div>
  ...
  `;
  document.getElementById('btn-export-csv').addEventListener('click', () => exportCsv(trail, agent));
  document.getElementById('btn-export-pdf').addEventListener('click', () => exportPdf(trail, agent));
}

function renderFlowCanvas(trail, agent) {
  const canvas = document.getElementById('audit-flow-canvas');
  const triggerHtml = `
    <div class="audit-flow-trigger">
      <div class="flow-node-icon" style="background:#5e6670;">M</div>
...
function exportCsv(trail, agent) {
  ...
  a.href = url;
  a.download = `audit-${trail.runId}-${slugifyName(agent.name)}.csv`;
...
function slugifyName(name) {
  return (name || 'agent').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
...
function exportPdf(trail, agent) {
  ...
  doc.innerHTML = `
    ...
    <div class="p-summary">
      <div><strong>Agent:</strong> ${escapeHtml(agent.name)}</div>
